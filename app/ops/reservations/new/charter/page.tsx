...
import { CharterReservationForm } from "./CharterReservationForm";
...
export default async function CharterReservationPage() {
  ...
  const flights = ((data ?? []) as FlightRow[]).filter(
    (f) => (f.seats_available ?? 0) > 0 && (f.seats_blocked ?? 0) === 0
  );
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold">Charter Reservation</h1>
        <div className="mt-1 text-sm text-slate-700">
          Capture charter reservation details and reserve the full aircraft.
        </div>
      </div>

      <Card
        title="New Charter Reservation"
        subtitle="Operational charter reservation capture aligned to the current airline workflow."
      >
        <CharterReservationForm flights={flights} />
      </Card>

      <Card
        title="Eligible Flights"
        subtitle="Flights currently available for full aircraft charter."
      >
        {flights.length === 0 ? (
          <div className="rounded-xl border bg-slate-50 p-6 text-sm text-slate-700">
            No flights currently available for charter.
          </div>
        ) : (
